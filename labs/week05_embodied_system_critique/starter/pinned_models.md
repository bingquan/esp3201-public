...
### Additional bugs found and fixed by running the notebook end-to-end (not just reading it)

- **Path bug (both the rollout-bank cell and the VAE round-trip's fallback):** `rollout_bank/` is a sibling of `starter/`, not a child of it. From `starter/notebooks/` (Jupyter's default cwd, the notebook's own directory) the correct relative path is `../../rollout_bank`, not `../rollout_bank`. The original `../rollout_bank` candidate silently matched zero files (empty glob, no error) rather than crashing, which is why it wasn't caught until frame counts were checked, not just exit codes.
- **VAE round-trip crash on non-8-divisible input:** if the notebook's `frames` variable holds a rollout-bank frame (213x384 -- not divisible by 8) when Exercise 1 runs, encoding then decoding through the VAE returns a slightly different size (208x384) than the input, and a direct pixel-diff crashes with a numpy shape-mismatch error. Fixed by cropping the input to the nearest multiple of 8 before encoding, and comparing against the cropped version.
- **`export_to_video`'s numpy-array path assumes float [0,1] input** and unconditionally multiplies by 255 -- silently corrupting already-uint8 [0,255] frames via overflow wraparound. `video_eval.save_video()` sidesteps this by always converting frames to PIL Images itself before calling `export_to_video`, which has a separate (correct, no-rescale) code path for PIL input. `torch` is NOT required for this -- confirmed by blocking `torch` from being importable and calling `export_to_video` successfully; `diffusers`'s base install does not depend on it.
